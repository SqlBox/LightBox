...
alter_procedure
    : ALTER PROCEDURE procedure_name EDITIONABLE? NONEDITIONABLE? COMPILE DEBUG? compiler_parameters_clause* (REUSE SETTINGS)? ';'
    ;
...
create_procedure_body
    : CREATE (OR REPLACE)? EDITIONABLE? NONEDITIONABLE? PROCEDURE procedure_name ('(' parameter (',' parameter)* ')')?
...
alter_type
    : ALTER TYPE type_name EDITIONABLE? NONEDITIONABLE?
    (compile_type_clause
    | replace_type_clause
...
create_view
    : CREATE (OR REPLACE)? (OR? NO? FORCE)? EDITIONING? NONEDITIONABLE? EDITIONABLE? (EDITIONABLE EDITIONING)? VIEW
