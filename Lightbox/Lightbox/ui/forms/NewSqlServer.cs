...
namespace Lightbox.Forms.mysql
{
    public partial class NewSqlServer : Form
    {
        ...
        public NewSqlServer(LightboxdbDataSet.sql_serversRow server) : this()
        {
            bSave.Text = "Update";
            this.isUpdate = true;
            ...
            DbType type = Lightbox.core.sql.Utils._convert(server.db_type);
            if (Lightbox.core.sql.Utils.IsDbEmbedded(type) && string.IsNullOrEmpty(textBoxPath.Text))
            {
        ...
        {
            if (Lightbox.core.sql.Utils.IsDbEmbedded(db_type))
            {
        ...
        private Boolean performChecks()
        {
            ...
            DbType type = Lightbox.core.sql.Utils._convert(((ToolStripItemDbType)this.comboBoxDbTypes.SelectedItem).db_type);
            //port
            if (!string.IsNullOrEmpty(tbPort.Text))
            {
                try
            ...
            {
                if (!Lightbox.core.sql.Utils.IsDbEmbedded(type))
                {
        ...
        {
            LightboxdbDataSetTableAdapters.sql_serversTableAdapter adapter = new LightboxdbDataSetTableAdapters.sql_serversTableAdapter();
